...
\begin{flushright} \large
	...
	\mynotes{Joseph Tran}
%Si j'avais mis juste le nom de Fabienne, c'est parce qu'elle a signé les papiers officiels. Je ne vous oublie pas Delphine et toi.
\end{flushright}
\end{minipage}
...
Si les résultats de l'analyse fournie par fastqc nous satisfont, on peut procéder au nettoyage des reads. Pour cela, on utilise l'outil Trimmomatic.
...
On aligne le chloroplaste de référence contre lui-même à l'aide de Nucmer. Nucmer est un pipeline de la suite MUMmer qui permet d'aligner extrêmement rapidement des génomes entiers contre une référence et de représenter ces alignements sous forme de graphe de ressemblance (dotplots).  
...
%C'est précisément parce que les mitos sont si différentes qu'on ne sait pas ce que va obtenir. Et cela ne s'arrête pas à la core collection cf l'article où on voit que les mitos de Ler, Col-0 et C24 n'ont rien à voir entre elles.
%Je suis d'acord, mais je l'aurais dit avec moins d'assurance, avec plus de nuances parce que je ne sais vraiment pas ce qu'on va obtenir comme assemblages
...
\subfloat[Avant normalisation, on constate 2 pics, situés autour de 20 et 250 sur l'axe des abscisses. Cela signifie qu'on trouve dans notre jeu de données un grand nombre de 25-mers qui apparaissent autour de 20 fois et de 250 fois. Ce sont probablement des 25-mers qui appartiennent à la mitochondrie et au chloroplaste respectivement. Après normalisation, nous n'avons plus qu'un seul pic à 20 sur l'axe des abscisses. Les 25-mers mitochondriaux et chloroplastiques apparaissent donc désormais en moyenne 20 fois dans le jeu de données normalisé. Les couvertures de la mitochondrie et du chloroplaste ont été homogéinisées.]{
 ...
 \subfloat[La filtration permet de diminuer le nombre de 25-mers de très faible abondance sans dégrader les données. En effet, le pic d'abscisse 20 constaté à la figure précédente n'est pas modifié quel que soit le jeu de données. Les 25-mers mitochondriaux et chloroplastiques sont donc épargnés par la filtration. Ils apparaissent toujours entre 10 et 30 fois dans le jeu de données.]{
...
On effectue la recherche de la taille de k-mer optimale comme précédemment. On choisit k=71. La N50 est alors de 44kb. Il y a 90 contigs d'au moins 500 bases et le contig le plus long fait 85kb. \label{MV_brut}
...
\subfloat[Graphe de ressemblance entre la mitochondrie de référence en abscisse et les contigs obtenus (pour MetaVelvet, hash length 71, exp_covs 300_32, cov_cutoff 5) en ordonnée]{
...
%Ces contigs peu couverts sont sur le noyau, je ne le discute pas.
%Cependant, ils sont également d'après nucmer/blast bien présents chez la mitochondrie de référence.
%Il y a donc deux possibilités : soient ils sont présents sur la mito de Cvi-0 comme sur la mitochondrie de C24 et dans ce cas là, pourquoi leurs couvertures sont-elles si faibles ? soient ils ne sont pas présents sur la mito de Cvi-0 donc on a une délétion par rapport à la référence
\todo[inline]{ok, mais si ça mappe sur le noyau et la mito de reference, comment peut-on s'assurer de la provenance des reads si ce n'est en se fiant à la couverture/profondeur de séquençage? Et vu les niveaux de couverture, je penche pour une contamination nucléaire et je valide ton hypothèse d'une possible délétion chez Cvi-0 par arapport à la référence.}
\centerfloat
...
%L'idée d'utiliser le ratio était une approche naïve ; j'explique l'importance de la position des reads au paragraphe suivant.
...
On scaffolde les contigs obtenus avec MetaVelvet et décrits en détail à la section \ref{MV_brut}\todo{référence erronée ou manquante}.
...
On en déduit que, même lorsqu'on lui demande de ne pas scaffolder, Velvet exploite les informations sur les paires si elles sont présentes en utilisant Pebble pour fusionner certains contigs. Ce premier round de Pebble donne globalement \hlcomment{un meilleur assemblage}{la définition de meilleur assemblage n'est pas très bien définie, et la N50 n'est pas reconnu comme le critère infaillible loin de là\dots} mais il arrive que Pebble fasse des erreurs comme c'est le cas ici pour le chloroplaste. 

Lorsqu'on assemble les données brutes avec MetaVelvet, le premier round de Pebble est également lancé sur chacun des sous-graphes de MetaVelvet. Pourtant, comme nous l'avons vu à la section \ref{MV_brut}\todo{référence erronée ou manquante}, les trois régions du chloroplaste sont distinctes. Donc, Pebble commet l'erreur de relier les trois régions du chloroplaste uniquement pour les données normalisées mais pas pour les données brutes. 
...
\todo[inline]{Je crois qu'il faut pas s'emballer, il me semble que les résultats sont obtenus à partir des données normalisées, et à priori je dirais que l'explication réside dans le fait qu'on abaisse artificiellement la couverture des régions répétées ce qui aurait pour conséquence d'induire le scaffolder en erreur en considérant que le contig n'est pas répété. La statistique de Myers me semble un bon atout pour détecter les répétitions. Si on se refere aux résultats de velvet obtenus à partir des données brutes - ce qui devraient correspondre aux résultats de metavelvet - tu le dis toi même les contigs restent distincts. J'aimerais bien voir les résultats de sga-astat sur les contigs de metavelvet\dots}
...
%la biblio est incluse dans le nombre de pages du rapport (30) donc il va falloir gagner de la place
